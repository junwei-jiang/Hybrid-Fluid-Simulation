...
void DX12CudaInterop::LoadAssets()
{
	...
	// Create the vertex buffer.
	{
		...
		d3d12ResourceAllocationInfo = m_device->GetResourceAllocationInfo(getCurrLuidDeviceNodeMask(), 1, &CD3DX12_RESOURCE_DESC::Buffer(vertexBufferSize));
		...
		WindowsSecurityAttributes SemaphoreWindowsSecurityAttributes;
		LPCWSTR SemaphoreName = NULL;
		HANDLE SemaphoreSharedHandle;
		externalSemaphoreHandleDesc.type = cudaExternalSemaphoreHandleTypeD3D12Fence;
		m_device->CreateSharedHandle(m_fence.Get(), &SemaphoreWindowsSecurityAttributes, GENERIC_ALL, SemaphoreName, &SemaphoreSharedHandle);
		...
		attachExtBuffer("TSET", "Pos", sharedHandle, actualSize, SemaphoreSharedHandle);
...
void DX12CudaInterop::MoveToNextFrame()
{
	...
	waitExtBuffer("TEST", "Pos", currentFenceValue);

	updateParticle(1, 1, 1);
	m_fenceValues[m_frameIndex] = currentFenceValue + 1;

	signalExtBuffer("TEST", "Pos", m_fenceValues[m_frameIndex]);
